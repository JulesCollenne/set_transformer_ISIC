...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        attention = self.sigmoid(self.conv1(pool))
        x = x * attention
        return x
...
class CNNModel(nn.Module):
    def __init__(self, n_patients):
        ...
        self.fc2 = nn.Linear(128, n_patients)
...
model = CNNModel(n_patients).cuda()
...
for epoch in range(num_epochs):
    for images, labels in train_gen.train_data():
        ...
        loss = criterion(preds.unsqueeze(dim=1), lbls.unsqueeze(dim=1))
